...
def _enhance_phone_widget(field):
    """Turn a PhoneNumberField's single input into a tel input enhanced by
    intl-tel-input (see partials/intl_tel_input.html). The JS rewrites the
    value to E.164 on submit; server validation stays with the model field."""
    widget = field.widget
    widget.input_type = 'tel'
    widget.attrs['autocomplete'] = 'tel'
    widget.attrs['class'] = f"{widget.attrs.get('class', '')} js-intl-tel-input".strip()
...
class UserProfileForm(ModelForm, TailwindForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # phone is the single-input PhoneNumberField auto-generated by ModelForm
        # (region IT), enhanced client-side with intl-tel-input.
        _enhance_phone_widget(self.fields['phone'])
